...
class Proyectos extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('nom_pro', 'required'),
			array('nom_pro', 'length', 'max'=>50),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id_pro, nom_pro', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'lenguajesProyectoses' => array(self::HAS_MANY, 'LenguajesProyectos', 'id_pro'),
		);
	}
	...
	{
		return array(
			'id_pro' => 'Id del Proyecto',
			'nom_pro' => 'Nombre del Proyecto',
		);
	}
	
	public static function listProyectos(){
		$datos = CHtml::listData(Proyectos::model()				
				->together()
				->findAll(),'id_pro','nom_pro');
		return $datos;
	}
